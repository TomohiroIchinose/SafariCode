...
//		Debug.Log (jsonText);
	...
	void nori_rogic_ver2 (IList blocks, IList buildings)
	{
		...
		foreach (Dictionary<string,object> building in buildings) {	
			...
			sorted_building_list.Add (new sort_building(){block_ID=block_ID_Dic[block_name],building_name=building_name,width=width,height=height,x_pos=width/2,y_pos=2,z_pos=width/2});
		}
